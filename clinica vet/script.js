...
document.addEventListener('DOMContentLoaded', async () => {
  await VetCareDB.init();
  await VetCareEmail.init();
  initUI();
});
...
function initCalendar() {
  ...
  els.confirmBtn?.addEventListener('click', async () => {
    ...
    try {
      // 1️⃣ Salvar no Firebase (ou sessionStorage em demo)
      await VetCareDB.salvarAgendamento(dados);

      // 2️⃣ Disparar e-mails em paralelo
      await Promise.allSettled([
        email ? VetCareEmail.enviarParaCliente(dados) : Promise.resolve(),
        VetCareEmail.enviarParaClinica(dados)
      ]);

      // 3️⃣ Montar link WhatsApp da clínica
      if (els.whatsappBtn) els.whatsappBtn.href = VetCareWhatsApp.gerarLink(dados);

      // 4️⃣ Mostrar tela de sucesso
      els.summary.textContent = `${nome}${pet?` (pet: ${pet})`:''} — ${servico} em ${dataFormatada} às ${selectedTime}.`;
      els.timesSection.hidden=true; els.formFields.hidden=true; els.successBox.hidden=false;
      showToast('Agendamento confirmado! 🎉','sucesso');

    } catch(err) {
      console.error(err);
      showToast('Erro ao confirmar. Tente novamente.','erro');
    } finally {
